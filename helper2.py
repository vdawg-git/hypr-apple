...
def _base_split(imageData,  min_region_size, dominant_value ):
    ...
    def recursion(imageData):
        ...
        if h * w <= min_region_size or ( image == dominant_value ).sum() < 3  or ( image == dominant_value ).all():
...
def _split_and_merge(imageData, chunk_size, dominant_value):
    ...
    whites_removed = list( filter(lambda x: np.mean(x[0]) <= 128, merged) )
...
min_region_size = 45 # Adjust this parameter as needed
regions = _split_and_merge(( binary_image, (0, 0) ), min_region_size, dominant_value = 255)
